...
public class AreaImpl {
	...
	public static void main(String[] args) throws SQLException {
		...
		String sql = "SELECT * FROM region_table WHERE code LIKE '__' ORDER BY code ASC";
		...
		while (rs.next()) {
			...
			String selectCitySql = "SELECT * FROM region_table WHERE code LIKE '" + code + "__' ORDER BY code ASC";
			statement = msSqlConn.prepareStatement(selectCitySql);
			...
			while (rss.next()) {
				...
				String selectCountySql = "SELECT * FROM region_table WHERE code LIKE '" + cityCode + "__' ORDER BY code ASC";
				statement = msSqlConn.prepareStatement(selectCountySql);
				...
				while (rsss.next()) {
					...
					String selectStreetSql = "SELECT * FROM region_table WHERE code LIKE '" + countyCode + "___' ORDER BY code ASC";
					statement = msSqlConn.prepareStatement(selectStreetSql);
				...
				String selectCountySql1 = "SELECT * FROM region_table WHERE code LIKE '" + cityCode + "00___' ORDER BY code ASC";
				statement = msSqlConn.prepareStatement(selectCountySql1);
				ResultSet rsss1 = statement.executeQuery();
				while (rsss1.next()) {
					// 区县名称
					String countyName = rsss1.getString(1);
					// 自动补0
					String ks = StrUtils.padLeft(String.valueOf(k++), 6, '0');
					String ijks = ijs + ks;
					String countySql = "INSERT INTO ARE_AREA(ID, AREA_CODE, AREA_NAME, IS_DIS) VALUES(" + idWorker3.getId() + ", '" + ijks + "', '" + countyName + "', 0)";
					PreparedStatement statementsss = msSqlConn.prepareStatement(countySql);
					statementsss.executeUpdate();
